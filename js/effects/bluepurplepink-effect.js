/**
 * Gaming Generator - Blue Purple Pink Effect Module
 * 青→紫→ピンクエフェクトの処理を担当するモジュール
 */
 ...
 * 青→紫→ピンクエフェクト処理クラス
 */
export class BluePurplePinkEffect {
    /**
     * 青→紫→ピンクパレット
     */
    static COLORS = [
    ...
    static apply(imageData, settings = {}, progress = 0) {
        try {
            ...
            // 彩度調整されたカラーパレット
            const adjustedColors = BluePurplePinkEffect._adjustColorsForSaturation(
                BluePurplePinkEffect.COLORS,
                saturation / 100
            );
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                // グラデーション位置計算
                const position = BluePurplePinkEffect._calculateGradientPosition(
                ...
                // 線形補間でスムーズなグラデーション
                const blendedColor = BluePurplePinkEffect._interpolateColors(
                ...
                const luminance = BluePurplePinkEffect._calculateLuminance(originalR, originalG, originalB);
                const adjustedLuminance = Math.max(0.3, Math.min(1.0, luminance * 1.3));
            ...
            throw ErrorHandler.handle(new ErrorTypes.PROCESSING_ERROR(
                'Blue Purple Pink effect processing failed',
                error,
                { settings, progress }
            ));
        }
