/**
 * Gaming Generator - Rainbow Effect Module
 * 虹色エフェクトの処理を担当するモジュール
 */
    ...
    static apply(imageData, settings = {}, progress = 0) {
        try {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                // グラデーション位置計算
                const position = RainbowEffect._calculateGradientPosition(
                ...
                // 線形補間でスムーズなグラデーション
                const blendedColor = RainbowEffect._interpolateColors(
                ...
                const luminance = RainbowEffect._calculateLuminance(originalR, originalG, originalB);
                const adjustedLuminance = Math.max(0.3, Math.min(1.0, luminance * 1.4));
            ...
            throw ErrorHandler.handle(new ErrorTypes.PROCESSING_ERROR(
                'Rainbow effect processing failed',
                error,
                { settings, progress }
            ));
        }
